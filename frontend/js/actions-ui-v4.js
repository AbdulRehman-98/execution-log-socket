const ActionsRunUI = (function () {
    ...
    function createRun({ root, title }) {
        ...
        function updateRunStatus(force = false) {
            // If not forced, AND run is still in progress, do NOTHING.
            // Prevent premature pill change.
        ...
        function addStep(job, name, parentStep = null) {
            const step = {
                ...
                finish(status) {
                    ...
                    icon.classList.remove("gh-loader");   // remove spinner
                    icon.classList.remove("gh-status-icon");
                    ...
                    if (status === "success") {
                        ...
                        icon.className = "gh-status-icon"; // fallback
                    }
            ...
            row.className = "gh-step-row gh-step-running"; // start highlighted
            row.innerHTML = `
        ...
        function finishJob(job, status = "success") {
            ...
            // Update UI for job only
            const cfg = STATUS_COLORS[status] || STATUS_COLORS.in_progress;
            job.item.querySelector(".gh-job-status-text").textContent = cfg.text;
            // updateJobStatus(job);
        }
        ...
        function finishRun(statusOverride = null) {
            ...
            updateRunStatus(true); // force pill update NOW ONLY
        }
