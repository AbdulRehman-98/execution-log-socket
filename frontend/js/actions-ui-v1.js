const ActionsRunUI = (function () {
    ...
    const STATUS_COLORS = {
        queued:      { bg: '#6e7781', text: 'Queued',       icon: '●' },
        in_progress: { bg: '#0969da', text: 'In progress',  icon: '●' },
        success:     { bg: '#2da44e', text: 'Succeeded',    icon: '✓' },
        failed:      { bg: '#cf222e', text: 'Failed',       icon: '×' },
        skipped:     { bg: '#bf8700', text: 'Skipped',      icon: '●' }
    };
    ...
    function createRun({ root, title }) {
        ...
        function updateRunStatus() {
            ...
            run.headerPill.style.background = cfg.bg;
            run.headerPill.innerHTML = `<span>${cfg.icon}</span> <span>${cfg.text}</span>`;
        }
        ...
        function addStep(job, name, parentStep = null) {
            const step = {
                ...
                finish(status) {
                    ...
                    this.row.querySelector(".gh-status-icon").textContent = cfg.icon;
                    this.row.querySelector(".gh-status-icon").style.color = cfg.bg;
